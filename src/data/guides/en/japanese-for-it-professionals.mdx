---
title: "Japanese for IT Professionals & Software Engineers: A Working Guide"
description: "Practical Japanese for software engineers in Japan — phrases for standups, PR reviews, commit messages, sprint planning, demos, retros, and client calls, plus a week-1 survival deck and a JLPT-to-task ladder. Two-thirds of what vocab lists never tell you."
...
faqs:
  ...
  - q: "How do you say \"deploy\" or \"merge\" in Japanese?"
    a: "Depuroi (デプロイ) and ma-ji (マージ). The tokens are easy. The interesting choices are around them: depuroi shimasu (polite default), depuroi shitokimasu (casual same-side), depuroi sasete-itadakimasu (over-polite to client). Picking the right register is the actual skill."
...
Japanese for IT professionals — software engineers, SREs, QA, data — splits across two axes most vocab lists ignore. You need (a) the right **politeness register** — *keigo*, polite, or casual — for the moment, and (b) phrases for **engineer-specific rituals**: PR reviews, commit messages, standups, sprint planning, demos, retros, and client calls. This guide gives you both, plus a week-1 survival deck and a JLPT-to-task ladder.
...
The comment thread is only half of a PR. The **commit message** and the **description body** have their own register rules, and they're the opposite of what most people assume.

- **Commit messages:** keep them terse. English imperative is widely accepted on Japanese teams (`fix: handle null user`); if your team writes Japanese, stay verb-final and short — *ユーザーがnullの場合を修正* (fixed the null-user case). **Never add *keigo* to a commit message.** It's a log line, not a letter — *shūsei sasete-itadakimasu* in a commit reads as bizarre.
- **PR description body:** this is where register matters, but the trap is *over*-formalizing it. Write it in **plain polite (B)** — full *desu / masu* sentences, no heavy *keigo*. A reviewer reads it like a short status report, not a client email.

| Part | Register | Example |
|---|---|---|
| PR title | plain / B | *X no shūsei — Y no baai ni taiō* (Xの修正 — Yの場合に対応) |
| What & why | B | *Y no baai ni erā ga deta node, Z de handoru shimashita* (Yの場合にエラーが出たので、Zでハンドルしました) |
| Ask for review | B | *go-kakunin onegai-shimasu* (ご確認お願いします) |
| Flag a caveat | B | *X wa mikakunin desu — betsu PR de taiō yotei desu* (Xは未確認です — 別PRで対応予定です) |

> **Common mistake:** Writing the PR description in *keigo* as if it were a client email — *go-kakunin sasete-itadakitaku zonjimasu* (ご確認させていただきたく存じます). Overformal in a PR body reads as stiff and slightly anxious. Plain *desu / masu* is the norm; save *keigo* for the person you ping, not the description.
...
| Category | Sample terms |
|---|---|
| Verbs you'll use daily | *kakunin* (確認), *taiō* (対応), *shūsei* (修正), *kanryō* (完了), *kaishi* (開始), *kidōchū* (起動中) |
...
| Tech katakana | *taasuku* (タスク), *komitto* (コミット), *depuroi* (デプロイ), *ma-ji* (マージ), *bagu* (バグ), *risōsu* (リソース) |
...
| *depuroi* (デプロイ) | ___⌐ (heiban-ish) | Stress on "ploy" English-style |
| *ma-ji* (マージ) | ⌐_ (atamadaka) | Drawn-out, English "merge"-like |
| *bagu* (バグ) | ⌐_ (atamadaka) | English "bug" with rising tone |
...
*Depuroi* (デプロイ) and *ma-ji* (マージ). The tokens are easy. The interesting choices are around them: *depuroi shimasu* (polite default), *depuroi shitokimasu* (casual same-side), *depuroi sasete-itadakimasu* (over-polite to client). Picking the right register is the actual skill.
