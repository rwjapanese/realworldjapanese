...
| Date | Change | Author |
|---|---|---|
| 2026-06-16 | **CTR fix — EN title+description rewrite** (週次 GSC triage 2026-06-16: pos 9.6 / 48 impr / CTR 0%、1 ページ目下端で取りこぼし継続。7日窓でも 34 impr / 7.4 位 / 0% で一貫)。原因仮説：(a) 旧 title の "8 Scenarios and How to Spot a Soft No" の `and how to` 連結がややぎこちなく、(b) 差別化要素「soft no decode」が後半に埋もれてクリック判断時のフックにならない → **`+` 区切りで差別化を前出し**、**動詞句 "How to Spot" → 名詞 "the Soft No Decoder"** へ凝縮。title「How to Refuse Politely in Japanese: 8 Scenarios and How to Spot a Soft No」(73 chars)→「How to Refuse Politely in Japanese: 8 Scenarios + the Soft No Decoder」(69 chars)。description 「× 3 politeness tiers」を「× A/B/C politeness」へ揃え、「workplace scenarios」を明示。「how to say no」→「how to decline」で reader pain との同期。本文無変更。 | Claude Opus 4.7 + ryoooue |
