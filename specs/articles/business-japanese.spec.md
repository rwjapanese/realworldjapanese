...
| 2026-06-03 | **CTA/回遊 multiplier infra (not the deferred redesign).** Reference integration of two new reusable components: (1) `RelatedLinks.astro` wraps the JA 育成ハブ related-article groups as a responsive 2-col card grid (hover state, accent labels) — replaces the plain bulleted list; (2) `ProductCTA.astro` — a visually distinct, language-aware (ja/en) CTA box replacing the bare inline Gumroad paragraph. Site-wide default: `ProductCTA` auto-renders at article end from the (previously dead) `productCTA: "essential-30"` frontmatter via `PostDetails.astro`, so all productCTA articles get the box with zero per-file edits. **This hub article overrides that order per user pref (CTA before related links):** dropped its `productCTA` frontmatter (suppresses the auto end-CTA) and hand-placed `<ProductCTA lang="ja" />` immediately before the `## 関連記事` / `<RelatedLinks>` block, so the reader sees 配布教材 → 関連記事. `npm run build` green. | Claude Opus 4.8 + ryoooue |
